...
| **跑马灯** | `IsMessageMarqueeEnabled` + `MarqueeLabel` | AtomUI 扩展功能，长文本自动滚动 |
...
当 `IsMessageMarqueeEnabled = true` 时：
...
| 跑马灯 | ❌ 无 | ✅ `IsMessageMarqueeEnabled` | 🆕 AtomUI 扩展 |
...
| `Alert` | 四种类型 (`AlertType`)、图标展示 (`IsShowIcon`)、描述信息 (`Description`)、关闭功能 (`IsClosable` / `CloseIcon` / `CloseRequest`)、额外操作区 (`ExtraAction`)、跑马灯 (`IsMessageMarqueeEnabled`)、Design Token 集成 |
...
- **双消息展示器**：`Label` 和 `MarqueeLabel` 共存，通过 `IsMessageMarqueeEnabled` 切换可见性，实现跑马灯和普通文本的无缝切换。
