...
本文档覆盖 PopupConfirm 的控件实现、主题接入、状态同步和 Gallery 可见维护边界。具体属性注册、默认值、绘制细节和 AXAML selector 仍应直接阅读源码；本文只记录维护者必须理解的稳定结构和不变量。
...
- 内容与数据：`CancelText`、`ConfirmContent`、`ConfirmContentTemplate`、`Icon`、`OkText`、`Title`。
- 交互与状态：`ConfirmStatus`、`IsShowCancelButton`。
- 其他稳定入口：`OkButtonType`。
...
稳定事件路径包括 `Cancelled`、`Confirmed`、`PopupClick`。事件参数和触发时机属于兼容边界。
