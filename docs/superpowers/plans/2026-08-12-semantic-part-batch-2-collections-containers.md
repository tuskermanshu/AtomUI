...
- [x] **强制停止：** 保持 Expander 的所有实现改动未提交，直到用户验证真实宿主行为并明确授权提交。（2026-09-15：Gate B 与真机视觉验收完成后，用户明确授权提交（Gate C）；已创建单个控件家族提交 `f1a61683e`「feat(Semantic): 增加 Expander 语义部件并修复圆角裁剪失效」，涵盖 Semantic Part 改造与圆角裁剪修复，未推送。）
